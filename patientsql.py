...
con = sqltor.connect(host="localhost", user="root", password="Dsec@123") 
cur = con.cursor(buffered=True) 
cur.execute("CREATE DATABASE IF NOT EXISTS hello") 
cur.execute("USE hello") 
cur.execute(""" CREATE TABLE IF NOT EXISTS appt ( idno CHAR(12) PRIMARY KEY, name VARCHAR(100), age INT, gender CHAR(1), phone CHAR(10), bg VARCHAR(5) ) """) 
con.commit() 
def today_str(): 
...
def register_patient():
    ...
    cur.execute( "INSERT INTO appt (idno, name, age, gender, phone, bg) VALUES (%s, %s, %s, %s, %s, %s)", (idn, name, age, gender, phone, bg) ) 
    con.commit() 
...
DOCTOR_PASSWORDS = {str(i+1): 7001 + i for i in range(len(DOCTORS))}  # '1' -> 7001 etc. 
